...
def parseInput(inp):
    ...
    # (A1 A2, B1, B2, PrizeX, PrizeY)
    res = []
    inp = inp.split("\n\n")
    for machine in inp:
        machineNums = re.findall("\d+", machine)
        res.append(tuple(int(x) for x in machineNums))
    return res


def solution(inp, solution2=False):
    ans = 0
    inp = parseInput(inp)
    for machine in inp:
        ...
        if a1 / b1 == a2 / b2:
            ...
            if denomA == 0 or denomB == 0:
                # no solution
                continue
            aTokens = (p1 * b2 - p2 * b1) / denomA
            bTokens = (p1 * a2 - p2 * a1) / denomB
            if aTokens.is_integer() and bTokens.is_integer():
                # if aTokens < 100 and bTokens < 100:
                ans += 3 * aTokens + bTokens
...
if __name__ == "__main__":
    ...
    ans2 = solution(input, solution2=True)
    print(ans2)
