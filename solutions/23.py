...
def hasTsorted(node1, node2, node3):
    if node1[0] == "t" or node2[0] == "t" or node3[0] == "t":
        return sorted([node1, node2, node3])
    return []


def solution1(inp):
    graph = parseInput(inp)
    triangles = set()
            ...
            for neighbor1 in neighbors:
                for neighbor2 in neighbors:
                    if neighbor2 in graph[neighbor1]:
                        triangle = hasTsorted(node1, neighbor1, neighbor2)
                        if len(triangle) > 0:
...
def bronKerbosch(R, P, X, graph):
    ...
    u = next(iter(pUx))
    uNeighbors = graph[u]
    pMinusNeighbors = P - uNeighbors
    for v in pMinusNeighbors:
        cliques.extend(bronKerbosch(R | {v}, P & graph[v], X & graph[v], graph))
        P.remove(v)
        X.add(v)
    return cliques
...
def solution2(inp):
    graph = parseInput(inp)
    cliques = bronKerbosch(set(), set(graph.keys()), set(), graph)
    largestClique = max(cliques, key=len)
    print(largestClique)
    pw = sorted(list(largestClique))
    return ",".join(pw)
