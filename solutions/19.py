...
def hasMatch(pattern, towels, found):
    if len(pattern) == 0:
        return 1
    if pattern in found:
    ...
    for towel in towels:
        if pattern[: len(towel)] == towel:
            ans += hasMatch(pattern[len(towel) :], towels, found)
    found[pattern] = ans
    return ans
...
def solution1(inp):
    ans = 0
    towels, patterns = parseInput(inp)
    found = defaultdict(int)
    for pattern in patterns:
        if hasMatch(pattern, towels, found) > 0:
            ans += 1
    return ans
...
def solution2(inp):
    ...
    found = defaultdict(int)
    for pattern in patterns:
        ans += hasMatch(pattern, towels, found)
    return ans
